...
def start(update, context):
    ...
    update.message.reply_text("Для старту гри потрібно зареєструвати Ігрока\n")
...
def user (update, context):
    ...
    if not check_name(gamer_name) and not check_pin(gamer_pin):
        ...
        update.message.reply_text("Ігрок з таким Прізвищем та І'мям вже існує, введіть іншe ПІ\nабо якщо це саме Ви, введіть вірний пін-код")
    else:
        if gamer_name.title()+'\n' in gamer_scores and gamer_scores[gamer_scores.index(gamer_name.title()+'\n')+1]==gamer_pin+'\n':
            update.message.reply_text('Ласкаво просимо до гри '+gamer_name.title()+'. Ваш кращий результат: '+gamer_scores[gamer_scores.index(gamer_name.title()+'\n')+2])
        else:
            print(gamer_name.title(),file=fh)
            print(gamer_pin,file=fh)
            print('0',file=fh)
            update.message.reply_text('Ласкаво просимо до гри '+gamer_name.title()+'. Ви граєте перший раз, тому нагадуємо\n прочитати правила ги - /rules подивитесь топ-20 результатів - /scores\nпідказка від ШІ - /help остановити гру - /stop')
        update.message.reply_text('Почнемо гру...\n')
        fh.close()
        update.message.reply_text('Введіть будь-яке українське місто:\n')
        return 2
...
def scores (update, context):
    ...
    if print_string_max>20:
        print_string_max=20
    update.message.reply_text('     РЕКОРДИ ГРИ - МІСТА УКРАЇНИ -')  
    for x in range(print_string_max):
        update.message.reply_text(str(gamer_scores_for_print[x][0])+'  '+gamer_scores_for_print[x][1])
